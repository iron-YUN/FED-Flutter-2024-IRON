...
void main(){
  ...
  recommActor['공유']?['자동차']??"없음"
}이다!');
  showTxt('다음 이순신 시리즈가 있다면 추천배우는 조인성다!');
  showTxt('조인성의 취미는 ${
    recommActor['조인성']?['취미']}다! 사는곳은 ${
      recommActor['조인성']?['사는곳']
}다! 조인성의 자동차는 ${
  recommActor['조인성']?['자동차']??"없음"
}이다!');

// [다트문법 널세이프티 적용사례!!]
// -> Map 형 데이터일 경우 내부 속성 불일치 등으로
// null이 나올때 에러로 연결되므로 이런 경우에
// 다트는 필수적으로 널 세이프티 처리를 강제한다!
// 물음표(?) 하나는null 일 경우 "null"문자열 출력대체
// 물음표두개(??) 는 null발생시 문자열 대체한 경우
// 커스터마이징 대체문자열을 출력하게 해준다!

// 익명함수를 만들어서 특정함수에 값으로 전달한다!
japanShip((){
  print("댕....박....살..! 일본쉽새끼 침몰!");});
...
const recommActor = {
  '조인성':
  {'나이':42, '하비':'날기', '사는곳':'아무데나거지',"자동차":"붕붕이"},
  '공유':
...
String actorList(List list){
  ...
  // 리스트를 순회하는 for문
  // -> for(변수 in 리스트형변수){코드}
  for(var x in list ){ // x -list 의 개별값 순회
    temp += "\n🙄$x";
  }//// for ///////
  // 문자형으로 리턴값 설정
  return temp;
